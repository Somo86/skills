...
This guide includes upstream slash-command terminology for brownfield workflows. In Codex:
- Initialize with `specify init --here --ai codex --ai-skills`
- Use the installed `$speckit-*` skills for the core feature flow (`$speckit-specify`, `$speckit-plan`, `$speckit-tasks`, `$speckit-implement`)
- Perform brownfield-only operations such as codebase analysis, reverse-engineering, validation, and integration planning through this `sdd` skill as direct workspace workflows
- Prefer `.specify/` and generated feature paths over older `.speckit/` examples in this reference
...
**Command:**
```
brownfield analysis via this `sdd` skill
```
...
```
Analyze this codebase at moderate depth, focusing on:
...
```
# Surface analysis
Perform a surface-level analysis of this codebase

# Deep analysis
Perform a deep analysis including technical debt,
...
- Installs Codex skills in `.agents/skills/`
...
**Command:**
```
analyze codebase via this `sdd` skill
```
...
Read `.specify/memory/constitution.md` and present a summary highlighting:
...
└── existing/
    ├── 001-authentication/
    │   ├── spec.md            # Reverse-engineered requirements
...
**Next Step:** Optionally validate reverse-engineering, then proceed to Step 6 to specify your new feature
...
Specifications:
- specs/existing/001-authentication/spec.md
- specs/existing/001-authentication/plan.md
...
**Next Step:** Perform integration planning with this `sdd` skill before task generation
...
3. Manually edit specs in `specs/existing/`
...
| `integration planning` | Plan integration | After specify, before tasks |
